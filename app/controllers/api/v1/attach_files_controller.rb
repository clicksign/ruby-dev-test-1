  ...
  module V1
    class AttachFilesController < ApplicationController
      ...
      def create
        ...
        if @attach_file.save
          render :show, status: :created, locals: { attach_file: @attach_file }
        else
      ...
      def update
        if @attach_file.update(attach_file_params) && @attach_file.file&.attached?
          render :show, status: :ok, locals: { attach_file: @attach_file }
        else
          render json: @attach_file.errors, status: :unprocessable_entity
        end
      end
      ...
      def attach_file_params
        params.require(:attach_file).permit(:name, :folder_id, :file )
      end
    end
  end
end
