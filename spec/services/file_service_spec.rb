...
RSpec.describe FileService do
  describe '#create' do
    ...
    let(:file_service) { FileService.new(params) }
    ...
    context 'when file type is blob' do
      ...
      it 'creates Blob file' do
        expect{file_service.create}.to change{File::Blob.last}.to(
          have_attributes(name: file_name, file_data: 'io_object', folder: folder)
        )
      end
    end
    ...
    context 'when file is S3' do
      ...
      it 'creates s3 file' do
        expect{file_service.create}.to change{File::S3.last}.to(
          have_attributes(name:file_name, file_data: nil, attachment: have_attributes(service_name: 'amazon'), folder: folder)
        )
      end
    end
    ...
    context 'when file is Local' do
      ...
      it 'creates local file' do
        expect{file_service.create}.to change{File::Local.last}.to(
          have_attributes(name:file_name, file_data: nil, attachment: have_attributes(service_name: 'local'), folder: folder)
        )
      end
    end 
  end
  ...
  describe '#index' do
    ...
    let(:file) { create(:file_local, folder: folder) }
